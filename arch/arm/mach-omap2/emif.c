 ...
 */
static const struct lpddr2_timings *get_timings_table(const struct
						      lpddr2_timings *const
						      *device_timings, u32 freq)
{
...
static struct emif_regs *get_regs(u32 emif_nr, u32 freq)
{
	...
	if (!regs) {
		 ...
		 */
		for (i = 0; i < EMIF_MAX_NUM_FREQUENCIES && regs_cache[i];
		     i++) ;
